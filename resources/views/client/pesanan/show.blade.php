...
<div class="mb-6 animate-fade">
    ...
    <div class="flex items-center justify-between">
        ...
        <span class="{{ $pesanan->status_badge }}">{{ $pesanan->status }}</span>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        @if($pesanan->status == \App\Models\Pesanan::STATUS_MENUNGGU_DP && (!$dpLunas || !$dpInvoice))
        <div class="card bg-red-50 border-2 border-red-200 animate-slide-left delay-{{ $pesanan->status == \App\Models\Pesanan::STATUS_DIBATALKAN ? '200' : '100' }}">
            <div class="flex items-start gap-3">
                <div class="bg-red-600 rounded-lg p-2">
                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Detail Pesanan -->
        <div class="card animate-slide-left delay-{{ $pesanan->status == \App\Models\Pesanan::STATUS_DIBATALKAN ? '300' : ($pesanan->status == \App\Models\Pesanan::STATUS_MENUNGGU_DP && (!$dpLunas || !$dpInvoice) ? '200' : '100') }}">
            <div class="flex items-center gap-3 mb-4">
        ...
        @if($kuotaRevisi > 0)
        <div class="card animate-slide-left delay-{{ $pesanan->status == \App\Models\Pesanan::STATUS_DIBATALKAN ? '400' : ($pesanan->status == \App\Models\Pesanan::STATUS_MENUNGGU_DP && (!$dpLunas || !$dpInvoice) ? '300' : '200') }}">
        ...
        {{-- Preview --}}
        @if($pesanan->status != \App\Models\Pesanan::STATUS_DIBATALKAN && !empty($pesanan->preview_link))
            @if(!$dpLunas)
    ...
    <div class="space-y-6 sidebar-sticky">
        <!-- Status Timeline dengan 10 Status -->
        <div class="card animate-slide-right">
            ...
            <div class="space-y-4">
                @php
                    $statusOrder = $pesanan->status_order;
                    $currentStatus = $pesanan->status;
                    $isDibatalkan = $currentStatus === \App\Models\Pesanan::STATUS_DIBATALKAN;
                    
                    $allStatuses = [
                        ['order' => 1, 'name' => 'Pesanan Dibuat', 'const' => \App\Models\Pesanan::STATUS_PESANAN_DIBUAT],
                        ['order' => 2, 'name' => 'Menunggu Pembayaran DP', 'const' => \App\Models\Pesanan::STATUS_MENUNGGU_DP],
                        ['order' => 3, 'name' => 'Sedang Diproses', 'const' => \App\Models\Pesanan::STATUS_DIPROSES],
                        ['order' => 4, 'name' => 'Preview Siap', 'const' => \App\Models\Pesanan::STATUS_PREVIEW_SIAP],
                        ['order' => 5, 'name' => 'Revisi Diproses', 'const' => \App\Models\Pesanan::STATUS_REVISI_DIPROSES],
                        ['order' => 6, 'name' => 'Revisi Preview Siap', 'const' => \App\Models\Pesanan::STATUS_REVISI_PREVIEW_SIAP],
                        ['order' => 7, 'name' => 'Revisi Selesai', 'const' => \App\Models\Pesanan::STATUS_REVISI_SELESAI],
                        ['order' => 8, 'name' => 'Menunggu Pelunasan', 'const' => \App\Models\Pesanan::STATUS_MENUNGGU_PELUNASAN],
                        ['order' => 9, 'name' => 'Menunggu File Final', 'const' => \App\Models\Pesanan::STATUS_MENUNGGU_FILE_FINAL],
                        ['order' => 10, 'name' => 'Selesai', 'const' => \App\Models\Pesanan::STATUS_SELESAI],
                    ];
                @endphp
                ...
                @if($isDibatalkan)
                    ...
                    <div class="timeline-item flex items-start">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-xs text-gray-500">{{ $pesanan->dibatalkan_at ? $pesanan->dibatalkan_at->format('d M Y H:i') : $pesanan->updated_at->format('d M Y H:i') }}</p>
                        </div>
                    </div>
                @else
                    @foreach($allStatuses as $statusItem)
                        @php
                            $isCompleted = $statusOrder > $statusItem['order'];
                            $isCurrent = $currentStatus === $statusItem['const'];
                            $isPending = $statusOrder < $statusItem['order'];
                            ...
                            if ($isCompleted) {
                                ...
                                $bgColor = match($statusItem['const']) {
                                    \App\Models\Pesanan::STATUS_MENUNGGU_DP => 'bg-yellow-500',
                                    \App\Models\Pesanan::STATUS_DIPROSES => 'bg-blue-500',
                                    \App\Models\Pesanan::STATUS_PREVIEW_SIAP => 'bg-purple-500',
                                    \App\Models\Pesanan::STATUS_REVISI_DIPROSES => 'bg-orange-500',
                                    \App\Models\Pesanan::STATUS_REVISI_PREVIEW_SIAP => 'bg-indigo-500',
                                    \App\Models\Pesanan::STATUS_REVISI_SELESAI => 'bg-teal-500',
                                    \App\Models\Pesanan::STATUS_MENUNGGU_PELUNASAN => 'bg-amber-500',
                                    \App\Models\Pesanan::STATUS_MENUNGGU_FILE_FINAL => 'bg-cyan-500',
                                    default => 'bg-blue-500'
                                };
                            ...
                            $description = match($statusItem['const']) {
                                \App\Models\Pesanan::STATUS_PESANAN_DIBUAT => 'Pesanan berhasil dibuat',
                                \App\Models\Pesanan::STATUS_MENUNGGU_DP => $isCurrent ? 'Menunggu pembayaran DP' : ($isCompleted ? 'DP sudah dibayar' : 'Belum bayar DP'),
                                \App\Models\Pesanan::STATUS_DIPROSES => $isCurrent ? 'Tim sedang mengerjakan' : ($isCompleted ? 'Proses selesai' : 'Menunggu proses'),
                                \App\Models\Pesanan::STATUS_PREVIEW_SIAP => $isCurrent ? 'Preview siap dilihat' : ($isCompleted ? 'Preview sudah dilihat' : 'Menunggu preview'),
                                \App\Models\Pesanan::STATUS_REVISI_DIPROSES => $isCurrent ? 'Revisi sedang dikerjakan' : ($isCompleted ? 'Revisi selesai' : 'Belum revisi'),
                                \App\Models\Pesanan::STATUS_REVISI_PREVIEW_SIAP => $isCurrent ? 'Preview revisi siap' : ($isCompleted ? 'Preview revisi dilihat' : 'Menunggu preview revisi'),
                                \App\Models\Pesanan::STATUS_REVISI_SELESAI => $isCurrent ? 'Semua revisi selesai' : ($isCompleted ? 'Revisi completed' : 'Menunggu revisi'),
                                \App\Models\Pesanan::STATUS_MENUNGGU_PELUNASAN => $isCurrent ? 'Menunggu pelunasan' : ($isCompleted ? 'Sudah lunas' : 'Menunggu pelunasan'),
                                \App\Models\Pesanan::STATUS_MENUNGGU_FILE_FINAL => $isCurrent ? 'Menunggu file final' : ($isCompleted ? 'File final siap' : 'Menunggu file'),
                                \App\Models\Pesanan::STATUS_SELESAI => $isCurrent ? 'Pekerjaan selesai' : 'Menunggu selesai',
                                default => ''
                            };
                        @endphp
                        
                        <div class="timeline-item flex items-start">
                            <div class="flex-shrink-0 w-8 h-8 rounded-full {{ $bgColor }} flex items-center justify-center text-white">
                                {!! $icon !!}
                            </div>
                            <div class="ml-3">
                                <p class="text-sm font-medium {{ $isCurrent ? 'text-gray-900' : 'text-gray-700' }}">
                                    {{ $statusItem['name'] }}
                                </p>
                                <p class="text-xs {{ $isCurrent ? 'text-gray-600' : 'text-gray-500' }}">
                                    {{ $description }}
                                </p>
                                @if($isCurrent && $statusItem['order'] === 1)
                                    <p class="text-xs text-gray-500 mt-0.5">{{ $pesanan->created_at->format('d M Y H:i') }}</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
        ...
        <!-- Contact -->
        <div class="card bg-blue-50 border-2 border-blue-200 animate-slide-right delay-200">
            <div class="flex items-center gap-3 mb-3">
                <div class="bg-blue-600 rounded-lg p-2">
                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
