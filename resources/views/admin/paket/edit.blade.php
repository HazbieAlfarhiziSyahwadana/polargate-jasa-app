...
<div class="mb-6">
    ...
    <p class="text-gray-600">Update informasi paket</p>
</div>

<div class="card max-w-4xl">
    <form action="{{ route('admin.paket.update', $paket) }}" method="POST" x-data="paketForm()">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
            ...
            <div>
                ...
                <input type="text" name="nama_paket" id="nama_paket" value="{{ old('nama_paket', $paket->nama_paket) }}" class="input-field @error('nama_paket') border-red-500 @enderror" required>
                @error('nama_paket')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="harga" id="harga" value="{{ old('harga', $paket->harga) }}" class="input-field @error('harga') border-red-500 @enderror" required>
                @error('harga')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-4">
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3" class="input-field @error('deskripsi') border-red-500 @enderror" required>{{ old('deskripsi', $paket->deskripsi) }}</textarea>
            @error('deskripsi')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <!-- Fitur -->
        <div class="mt-4" x-init="fiturList = {{ json_encode(old('fitur', $paket->fitur)) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
            ...
            <div>
                ...
                <input type="number" name="durasi_pengerjaan" id="durasi_pengerjaan" value="{{ old('durasi_pengerjaan', $paket->durasi_pengerjaan) }}" class="input-field @error('durasi_pengerjaan') border-red-500 @enderror" required>
            ...
            <div>
                ...
                <input type="number" name="jumlah_revisi" id="jumlah_revisi" value="{{ old('jumlah_revisi', $paket->jumlah_revisi) }}" class="input-field @error('jumlah_revisi') border-red-500 @enderror" required>
                @error('jumlah_revisi')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-4">
            <label class="flex items-center">
                <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-primary-600" {{ old('is_active', $paket->is_active) ? 'checked' : '' }}>
